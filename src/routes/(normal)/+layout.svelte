...
{#snippet saveFilesBackupConfirmation()}
    <div class="flex flex-row items-center gap-2">
        ...
        <div class="text-sm">Save a backup of all files beforehand.</div>
    </div>
{/snippet}
            ...
            {#if filesystem.value?.filesWereModified}
                <div class="border-b-2 border-amber-500" transition:scale>
                    <Button disableLoading transparent class="[&]:p-1 w-8 h-8 rounded-none group relative" onclick={saveFiles}>
                        ...
                        <div
                            class="w-max font-normal z-50 absolute -bottom-10 py-1 px-2 left-1/2 -translate-x-1/2 bg-gray-700 opacity-0 invisible group-hover:visible group-hover:opacity-100 group-hover:delay-150 duration-150 shadow-2xl transition-all"
                        >
