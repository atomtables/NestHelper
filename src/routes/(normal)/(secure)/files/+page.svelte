    ...
    const refresh = () => {
        ...
        promise
            ...
            if (filesWithNoExistence.length > 0) {
                let [result] = await confirm("Deleted files detected", `The following files no longer exist and will be removed from the filesystem state: <br> ${filesWithNoExistence.map(f => `<b>${f}</b>`).join("<br>")} <br> Would you like to update, resulting in the deletion of your modified file changes?`);
                if (result) {
            ...
            .catch(error => {
                alert("Failed to load files", `There was an error loading the files. The server returned the following outputs: <div class="max-h-96 overflow-scroll"><br> STDOUT: <p class="text-red-500">${error?.stdout.replaceAll('\n', '<br>')}</p> <br> STDERR: <p class="text-red-500">${error?.stderr.replaceAll('\n', '<br>')}</p> <br></div>`);
