<script>
    ...
    const submit = () => {
        ...
        let ind =
            commandHistory.value.history.push({
                id: currentCommand.id,
                command: value,
                timestamp: new Date(),
            }) - 1;
        let history = commandHistory.value.history;
        currentCommand.promise
            .then(() => {
                        ...
                        <span class="text-xs text-gray-300">
                            ...
                            • {@html cmd?.returnCode === 0
                                ...
                                : cmd?.returnCode !== undefined
                                  ? `<span class="text-red-400">exit code ${cmd.returnCode}</span>`
        ...
        <div class="flex flex-col items-center w-full flex-4/5">
            <div class="flex flex-col w-full grow overflow-y-scroll scroll-auto" bind:this={scrollContainer}>
                ...
                {:else}
                    <div class="w-full h-full flex flex-col items-center justify-center">
                        <div class="text-2xl">Hey, forget that delay typing into SSH.</div>
                        <div class="text-base text-gray-400">Just run commands directly here.</div>
                    </div>
                {/if}
            </div>
            <div class="w-full px-4 shrink-0">
                <div class="bg-neutral-900 w-full flex flex-col items-center justify-center rounded-t-xl px-2">
                    ...
                    <div class="flex flex-row items-center justify-center w-full pb-2">
                        <Input noautomodify bind:value name="Command" class="pr-2" onkeydown={(e) => e.key === 'Enter' && value.length >= 1 && submit()} />
