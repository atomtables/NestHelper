...
<div class="p-2 font-sans">
    ...
    <div>
        Success is triggered when the promise you write resolves. The return value you provide has no meaning. If you want to error out your task (for example,
        if a condition failed), throw an Error normally.
    </div>
    <br />
    ...
    <div>
        You can output to the Flow's output by using the provided functions <b>log</b> and <b>logError</b>. Do note that output from a JavaScript task is not
        accessible by any other task, and is only for visual inspection. You can still use <b>console.log</b> to get more debug output.
    </div>
    <div class="bg-gray-800 text-white p-2 rounded-lg shadow-md m-2">
    ...
    <div>
        ...
        function, which will make the request through the backend rather than through the JavaScript engine. This helper function is less featured than the native
        fetch, so use <b>window.fetch</b> when possible.
    </div>
    <div class="bg-gray-800 text-white p-2 rounded-lg shadow-md m-2">
        <pre class="overflow-x-auto"><code class="language-javascript"
