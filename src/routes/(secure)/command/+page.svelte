<script>
    ...
    import {commandHistory, filesystem} from "$lib/state/states.svelte.js";
    ...
    import {save as saveState} from "$lib/state/states.svelte.js"
    ...
    const submit = () => {
        ...
        commandHistory.value = (commandHistory.value instanceof Array && commandHistory.value) || []
        let ind = commandHistory.value.push({
            id: currentCommand.id,
            command: value,
            timestamp: new Date()
        }) - 1
        currentCommand.promise.then(() => {
            commandHistory.value[ind].returnCode = currentCommand.returnCode
            commandHistory.value[ind].stdout = currentCommand.stdout
            commandHistory.value[ind].stderr = currentCommand.stderr
            commandHistory.value[ind].output = currentCommand.htmlOutput
            commandHistory.value[ind].outputArr = currentCommand.output
        }).catch(() => {
            commandHistory.value[ind].returnCode = currentCommand.returnCode
            commandHistory.value[ind].stdout = currentCommand.stdout
            commandHistory.value[ind].stderr = currentCommand.stderr
...
<div class="w-full h-full flex flex-row flex-nowrap">
    <div class="flex-1/4 flex flex-col items-start w-full gap-0 p-2 bg-purple-900 overflow-y-auto scroll-y-auto">
        ...
        {#if commandHistory.value && commandHistory.value instanceof Array}
            {#each reversed(commandHistory.value) as cmd, i (commandHistory.value.length - i)}
                ...
                >
                    <span class="text-xs text-gray-300">
                        {dateIsSameDay(new Date(), cmd.timestamp) ? cmd.timestamp.toLocaleTimeString() : cmd.timestamp.toLocaleString()}
            ...
            <div class="bg-neutral-900 w-full flex flex-col items-center justify-center rounded-t-xl px-2">
                <div class="flex flex-row items-center justify-start gap-2 w-full pt-2 transition-all">
                    ...
                    {#if currentCommand && currentCommand?.state !== "ongoing"}
                        <div transition:slide={{ duration: 150 }} >
                            <button onclick={() => {commandHistory.value = commandHistory.value.filter(v => v.id !== currentCommand.id); currentCommand = null}} class="group relative w-8 h-8 bg-purple-800 hover:bg-purple-700 active:bg-purple-600 cursor-pointer flex items-center justify-center rounded-full">
