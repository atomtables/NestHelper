...
<script>
    ...
    import {app} from "$lib/state/states.svelte.js";
    import {Command} from "$lib/conn/Command.js";

    let { result, dir } = $props()
    ...
    let folderPath = $state([dir]);
    // noinspection JSUnresolvedReference
    let currentFolder = $derived.by(() => {
        let folder = result.children;
        for (let i = 1; i < folderPath.length; i++) {
            console.log(`Navigating to ${folderPath[i]}`);
            folder = folder[folderPath[i]].children;
        }
        return folder;
    })
    ...
    const loadFile = async () => {
        let filePath = folderPath.join('/') + '/' + selectedFile[0];
        ...
        try {
            ...
            console.log(arr, currentFile)
            app.value.status = `File loaded: ${filePath}`;
        } catch (e) {
            console.error("Error loading file:", e);
            await alert("Error loading file", `There was an error loading your file: "${e.message}". ${e instanceof TypeError ? "Please choose the correct type for the file you wish to view." : ""}`);
        } finally {
            app.value.status = "";
        }
    }
...
{#if result}
    <div class="w-full h-full flex flex-row flex-nowrap inset-0">
        ...
        <div class="flex-3/4 flex flex-col justify-center items-center m-4 overflow-auto scroll-auto">
            {#if currentFile}
                <pre class="w-full h-full">{currentFile}</pre>
            {:else}
                {#if selectedFile}
