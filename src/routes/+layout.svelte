...
<div class="h-(--top-bar) z-50 bg-purple-200 dark:bg-purple-900 flex flex-row justify-between items-center backdrop-blur-sm" data-tauri-drag-region>
	{#if appReady.value.persistentStoresLoaded && auth.value.username}
		<div class="rounded-2xl flex flex-row items-center h-8 select-none">
			...
			<a href="/main" class="transition-colors flex flex-row items-center hover:bg-purple-300 dark:hover:bg-purple-800 active:bg-purple-400 dark:active:bg-purple-700 rounded-full p-2">
				<img src={image} alt="Relaunch the app" class="h-4 w-4" />
			</a>
			<a href="/flow" class="transition-colors flex flex-row items-center hover:bg-purple-300 dark:hover:bg-purple-800 active:bg-purple-400 dark:active:bg-purple-700 rounded-full p-2">
				<img src={image} alt="Relaunch the app" class="h-4 w-4" />
			</a>
		</div>
